...
const API_KEY = process.env.HOKUSAI_API_KEY
const CONTRACT_ID = process.env.HOKUSAI_CONTRACT_ID
...
const data = JSON.parse(fs.readFileSync('data.json', 'utf8')) as { key: number, hash: string, address: string }[]

const main = async () => {
  try {
    for (const item of data) {
      const url = "ipfs://" + item.hash
      console.log(`     ${item.key}: ` + url)
      const res = await mintNft(baseUrl, API_KEY, CONTRACT_ID, item.address, url)
      if (res.txHash) {
